...
const Chat = ({ user, setUser }) => {
  const { messages, reactions, isConnected, sendMessage, sendReaction } = useWebSocket(user);
  ...
  const handleReaction = useCallback((messageTime, messageIndex, emoji) => {
    setActiveReactionPicker(null);
    sendReaction(messageTime, messageIndex, emoji, user);
  }, [user, sendReaction]);
  ...
  return (
    <div className="flex flex-col h-screen w-full">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              <span className={`w-1.5 h-1.5 rounded-full shrink-0 transition-colors duration-300 ${isConnected ? 'bg-[#00a884]' : 'bg-yellow-400 animate-pulse'}`} />
      ...
      <main
        ref={boxRef}
        className="flex-1 overflow-y-auto chat-wallpaper px-2 sm:px-4 py-3 scroll-smooth [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-white/10 [&::-webkit-scrollbar-track]:bg-transparent"
      >
        {messages.length === 0 ? (
      ...
      <footer className="shrink-0 px-2 sm:px-4 py-2.5 flex items-center gap-2 bg-[#1f2c34] border-t border-white/5">
        ...
        <input
          ...
          autoComplete="off"
          className="flex-1 rounded-xl px-4 py-2.5 text-sm text-white placeholder-gray-500 bg-[#2a3942] border border-transparent focus:outline-none focus:ring-2 focus:ring-[#00a884]/20 focus:border-[#00a884]/30 transition-all disabled:opacity-50"
        />
        ...
        <button
          ...
          className={`shrink-0 w-10 h-10 rounded-full flex items-center justify-center transition-all cursor-pointer border-none
            ${text.trim() && isConnected ? 'bg-[#00a884] hover:bg-[#00c49a]' : 'bg-white/[0.06] cursor-not-allowed'}
            ${sendAnimating ? 'animate-send-pulse' : ''}`}
        >
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
            className={`transition-opacity ${text.trim() && isConnected ? 'opacity-100' : 'opacity-30'}`}
